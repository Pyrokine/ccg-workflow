...
用 `TaskOutput` 等待结果。**📌 保存 SESSION_ID**（`CODEX_SESSION` 和 `FRONTEND_SESSION`）。
...
- {{FRONTEND_PRIMARY}}：使用规划提示词 + `resume $FRONTEND_SESSION`，输出前端架构

用 `TaskOutput` 等待结果。

**务必遵循上方 `多模型调用规范` 的 `重要` 指示**

**Claude 综合规划**：采纳 {{BACKEND_PRIMARY}} 后端规划 + {{FRONTEND_PRIMARY}} 前端规划，用户批准后存入
`.claude/plan/任务名.md`
