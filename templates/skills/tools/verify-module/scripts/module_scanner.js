  ...
  try {
    for (const name of fs.readdirSync(dir)) {
      ...
      try {
        ...
        if (stat.isDirectory()) {
          if (rglob(full, test)) return true;
        }
      } catch {}
    }
  } catch {}
...
function scanModule(target) {
  ...
  let srcFound = ALT_SRC_DIRS.some((d) => {
    try {
      return fs.statSync(path.join(modulePath, d)).isDirectory();
    } catch {
      return false;
    }
  });
  ...
  const rootScript = entries.filter((n) => {
    try {
      return fs.statSync(path.join(modulePath, n)).isFile() && ROOT_SCRIPT_FILES.has(n);
    } catch {
      return false;
    }
  });
  ...
  if (fs.existsSync(readme)) {
    ...
    if (!docKeys.some((k) => c.toLowerCase().includes(k))) add('info', 'README.md 建议添加使用说明或示例', readme);
  }
  ...
  if (fs.existsSync(design)) {
    ...
    if (!designKeys.some((k) => c.toLowerCase().includes(k))) add('info', 'DESIGN.md 建议记录设计决策和权衡', design);
  }
