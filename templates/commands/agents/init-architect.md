...
- **忽略规则获取策略**：
    ...
    2. 如果 `.gitignore` 不存在，则使用以下默认忽略规则：
       `node_modules/**,.git/**,.github/**,dist/**,build/**,.next/**,__pycache__/**,*.lock,*.log,*.bin,*.pdf,*.png,*.jpg,*.jpeg,*.gif,*.mp4,*.zip,*.tar,*.gz`
...
1. **阶段 A：全仓清点（轻量）**
    - 以多次 `Glob` 分批获取文件清单（避免单次超限），做：
        - 文件计数、语言占比、目录拓扑、模块候选发现（package.json、pyproject.toml、go.mod、Cargo.toml、apps/_、packages/_
          、services/_、cmd/_ 等）。
...
2. **写入模块级 `CLAUDE.md`**
    - 放在每个模块目录下，结构建议：
        ...
        - 变更记录 (Changelog)
3. **`.claude/index.json`**
    - 记录：当前时间戳（通过参数提供）、根/模块列表、每个模块的入口/接口/测试/重要路径、**扫描覆盖率**、忽略统计、是否因上限被截断（
      `truncated: true`）。
...
- 模块列表（路径+一句话职责）；
- 覆盖率与主要缺口；
- 若未读全：说明"为何到此为止"，并列出**推荐的下一步**（例如"
  建议优先补扫：packages/auth/src/controllers、services/audit/migrations"）。
