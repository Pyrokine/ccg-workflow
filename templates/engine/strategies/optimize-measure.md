...
3. 输出基线：
   ...
   📊 性能基线
     ...
     目标: [用户期望值，如有]
   ```
4. 使用 `write-artifact` 把基线与测试方法写入 `analysis.md`，再用 `checkpoint` 更新为 `1-baseline`
...
3. 按影响大小排序瓶颈：
   ...
   🔍 瓶颈分析
     ...
     2. [位置] — 预估影响: [高/中/低] — [原因]
   ```
4. 更新 `analysis.md` 并用 `checkpoint` 记录 `2-analyze`，下一动作设为“优化排名最高且可验证的瓶颈”

### Phase 3: 针对性优化

**一次优化一个瓶颈**（便于验证每个优化的效果）：

1. 使用 `checkpoint` 更新为 `3-optimize`，把 `nextAction` 写成当前瓶颈与验证方法
2. 应用优化
3. 简要说明做了什么
4. 如果涉及算法变更，确保正确性
...
4. 用 `checkpoint` 记录 `4-measure` 和基线对比结果
5. 按 `phase-guide.md § 7` 记录 Spec Evolution；任务达到用户目标后调用 `finish`，未达到时回到 Phase 2
