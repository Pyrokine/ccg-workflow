...
describe('impeccable is collapsed into a single frontend-design entry', () => {
  ...
  it('supporting playbooks do not advertise their retired slash commands', () => {
    const dead: string[] = []
    for (const skill of impeccable) {
      const text = fs.readFileSync(skill.skillPath, 'utf-8')
      for (const candidate of impeccable) {
        const re = new RegExp(`(?<![\\w/])/(?:ccg:)?${candidate.name}\\b`, 'g')
        if (re.test(text)) dead.push(`${skill.name}:/${candidate.name}`)
      }
    }
    expect(dead).toEqual([])
  })
