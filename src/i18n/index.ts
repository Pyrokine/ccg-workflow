...
const zhCN = {
  ...
  init: {
    ...
    api: {
      ...
      sponsorCodexActivatePrompt: '立即将 Codex 默认切换到 {{name}}？（选否仅注册，保留现有 Codex 账号计费）',
      sponsorCodexEnvHint: 'Codex 通过环境变量读取密钥，请加入 shell 配置：',
      sponsorCodexNotActive: '仅注册未启用，需要时在 ~/.codex/config.toml 设置 model_provider = "{{id}}"',
...
const en: typeof zhCN = {
  ...
  init: {
    ...
    api: {
      ...
      sponsorCodexActivatePrompt:
        'Switch Codex to {{name}} now? (No only registers it and keeps current Codex billing)',
      sponsorCodexEnvHint: 'Codex reads the key from the environment; add this to your shell profile:',
      sponsorCodexNotActive:
        'Registered but inactive; set model_provider = "{{id}}" in ~/.codex/config.toml when needed',
