...
const zhCN = {
  ...
  init: {
    ...
    api: {
      ...
      sponsorAPIMart: 'APIMart（赞助商 · Claude 与 GPT API 网关）',
      sponsorAPIMartGetKey: '注册并获取 API Key',
      apimartCodexPrompt: '同时把 APIMart 注册为 Codex CLI 的模型供应商？',
      apimartCodexActivatePrompt: '立即将 Codex 默认切换到 APIMart？（选否仅注册，保留现有 Codex 账号计费）',
      apimartCodexEnvHint: 'Codex 通过环境变量读取密钥，请加入 shell 配置：',
      apimartCodexNotActive: '仅注册未启用，需要时在 ~/.codex/config.toml 设置 model_provider = "apimart"',
    ...
    model: {
      ...
      geminiDisabled:
        'Gemini CLI 已禁用：2026-06-18 后 consumer OAuth 请求不再处理，CCG 将旧 Gemini 路由迁移为 Grok，agy 保留为 Antigravity 别名。',
      grokModel: 'Grok 模型名称',
      kimiModel: 'Kimi Code 模型名称（留空使用 CLI 默认值）',
      opencodeModel: 'OpenCode 模型（provider/model，留空使用默认值）',
      modelRequired: '请输入模型名称',
      pureClaudeCode:
        '纯 Claude Code 模式已启用，前端和后端不会启动外部模型 CLI；GPT、Grok 审查仍通过 codeagent-wrapper 的独立临时会话执行',
      routingBackup: '已备份 {{count}} 个被路由刷新覆盖的文件: {{path}}',
      recommended: '推荐',
      currentRouting: '当前模型路由',
      currentFrontend: '前端',
      currentBackend: '后端',
      currentGrok: 'Grok',
      currentKimi: 'Kimi Code',
      currentOpencode: 'OpenCode',
      routingUpdated: '模型路由已更新',
      reinstalling: '正在刷新路由模板和 Hook runtime...',
      reinstallDone: '路由模板和 Hook runtime 已刷新',
      reinstallFailed: '路由模板或 Hook runtime 刷新失败',
    },
    perf: {
      title: '性能模式',
      selectMode: '选择性能模式',
    ...
    commands: {
      includeImpeccable: '安装 frontend-design 前端设计技能？（内含 20 个 Impeccable UI/UX playbook）',
    },
    hooks: {
  ...
  menu: {
    ...
    help: {
      ...
      descriptions: {
        ...
        specImpl: '规范驱动实现',
        specReview: '归档前 GPT、Grok 双 profile 审查',
        commit: 'Git 智能提交',
        rollback: 'Git 交互式回滚',
...
const en: typeof zhCN = {
  ...
  init: {
    ...
    api: {
      ...
      sponsorAPIMart: 'APIMart (Sponsor · Claude and GPT API gateway)',
      ...
      apimartCodexActivatePrompt: 'Switch Codex to APIMart now? (No only registers it and keeps current Codex billing)',
      apimartCodexEnvHint: 'Codex reads the key from the environment; add this to your shell profile:',
      apimartCodexNotActive:
        'Registered but inactive; set model_provider = "apimart" in ~/.codex/config.toml when needed',
    ...
    model: {
      ...
      geminiDisabled:
        'Gemini CLI is disabled: consumer OAuth requests stopped being processed after 2026-06-18; CCG migrates legacy Gemini routes to Grok and retains agy as an Antigravity alias.',
      grokModel: 'Grok model name',
      kimiModel: 'Kimi Code model name (leave empty for the CLI default)',
      opencodeModel: 'OpenCode model (provider/model, leave empty for the default)',
      modelRequired: 'Enter a model name',
      pureClaudeCode:
        'Pure Claude Code mode is enabled; frontend and backend external model CLIs will not start, while GPT and Grok reviews still use independent temporary sessions through codeagent-wrapper',
      routingBackup: 'Backed up {{count}} routing templates: {{path}}',
      ...
      reinstalling: 'Refreshing routing templates and Hook runtime...',
      reinstallDone: 'Routing templates and Hook runtime refreshed',
      reinstallFailed: 'Routing template or Hook runtime refresh failed',
    },
    perf: {
    ...
    commands: {
      includeImpeccable: 'Install the frontend-design skill? (includes 20 Impeccable UI/UX playbooks)',
    },
    hooks: {
  ...
  menu: {
    ...
    help: {
      ...
      descriptions: {
        ...
        specReview: 'Pre-archive GPT/Grok review',
